...
export async function validateRequiredArtwork(userId: string, productId: string, configuration: Record<string, unknown>) {
  ...
  const [artwork] = await db.select({ id: artworks.id }).from(artworks).where(and(eq(artworks.id, artworkId), eq(artworks.uploadedBy, userId), eq(artworks.productId, productId), pricingRuleId ? eq(artworks.pricingRuleId, pricingRuleId) : isNull(artworks.pricingRuleId))).limit(1);
