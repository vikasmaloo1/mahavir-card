...
export async function getOwnedCart(userId: string, kind: CartKind) {
  ...
  if (!cart) return { id: null, kind, items: [], summary: { itemCount: 0, priceBeforeTax: "0.00", tax: "0.00", total: "0.00", currency: "INR", taxInclusive: true, hasTaxBreakdown: false, hasUnavailableItems: false } };
  ...
  return {
    ...
    summary: {
      ...
      priceBeforeTax: (Number(total) - tax).toFixed(2),
      tax: tax.toFixed(2),
      total,
