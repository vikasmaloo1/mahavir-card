      ...
      {data.items.length ? data.items.map((item) => <article key={item.id} className="rounded-xl border border-[var(--mc-line)] bg-white p-4 shadow-sm sm:p-5">
        <div className="flex gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            {item.pricingSnapshot.addons?.length || item.pricingSnapshot.delivery?.method ? <div className="mt-2.5 flex flex-wrap gap-x-4 gap-y-1 text-sm text-[var(--mc-muted)]">{item.pricingSnapshot.addons?.length ? <span>Add-ons: {item.pricingSnapshot.addons.map((addon) => addon.name).join(", ")}</span> : null}{item.pricingSnapshot.delivery?.method ? <span>Delivery: {item.pricingSnapshot.delivery.method.replaceAll("_", " ")}</span> : null}</div> : null}
