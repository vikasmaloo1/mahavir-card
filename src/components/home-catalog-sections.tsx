...
type Product = { id: string; name: string; slug: string; shortDescription: string | null; imageUrl: string | null; category: { name: string } | null; priceLabel: string; priceState: "STARTING" | "CUSTOM_QUOTE" | "CONTACT"; taxInclusive: boolean | null; orderable: boolean; quoteable: boolean };

const featuredSlugs = ["business-card-tearable-single-side", "brochures", "product-labels", "packaging-boxes", "notebooks", "paper-bags"];
  ...
  return <>
    ...
    <section className="border-y border-[var(--mc-line)] bg-[#f0f5ff]">
      <div className="mx-auto max-w-[1440px] px-4 py-12 lg:px-8 lg:py-16">
        ...
        {featured.length ? <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{featured.map((product) => <article key={product.id} className="overflow-hidden rounded-lg border border-[var(--mc-line)] bg-[var(--mc-paper)] shadow-[0_12px_34px_rgba(16,33,63,0.08)] transition hover:-translate-y-0.5 hover:border-[#a9c2ef] hover:shadow-[0_18px_42px_rgba(40,100,220,0.14)]"><Link href={`/catalog/${product.slug}`} className="relative block aspect-[1.72] overflow-hidden bg-white"><ProductImage src={product.imageUrl || "/images/mahavir-print-assortment.png"} alt={`${product.name} print sample`} slug={product.slug} /><span className="absolute left-3 top-3 rounded-full bg-[var(--mc-paper)]/95 px-3 py-1.5 text-xs font-bold uppercase text-[var(--mc-accent)] shadow-sm">{product.category?.name ?? "Print product"}</span></Link><div className="p-5"><h3 className="text-xl font-bold text-[var(--mc-ink)]">{product.name}</h3><p className="mt-2 min-h-12 text-[15px] leading-6 text-[var(--mc-muted)]">{product.shortDescription || "Configure the print specification and quantity."}</p><div className="mt-5 border-t border-[var(--mc-line)] pt-4"><p className="text-xs font-bold uppercase text-[var(--mc-muted)]">{product.priceState === "STARTING" ? "Online price" : "Pricing"}</p><p className="mt-1.5 text-xl font-bold text-[var(--mc-ink)]">{product.priceLabel}</p>{product.taxInclusive ? <p className="mt-1 text-xs font-medium text-[var(--mc-muted)]">GST included</p> : null}</div><div className="mt-5 flex flex-wrap gap-2.5"><Link href={`/catalog/${product.slug}`} className="inline-flex items-center gap-1.5 bg-[var(--mc-accent)] px-4 py-2.5 text-sm font-bold text-white">Configure <ArrowRight size={16} /></Link>{product.orderable ? <Link href={`/catalog/${product.slug}?intent=buy`} className="inline-flex items-center gap-1.5 border border-[var(--mc-accent)] px-4 py-2.5 text-sm font-bold text-[var(--mc-accent)]"><ShoppingBag size={16} />Buy now</Link> : null}{product.quoteable ? <Link href={`/catalog/${product.slug}?intent=quote`} className="inline-flex items-center gap-1.5 border border-[var(--mc-line)] px-4 py-2.5 text-sm font-bold text-[var(--mc-muted)]"><FileText size={16} />Quote</Link> : null}</div></div></article>)}</div> : <div className="mt-8 rounded-lg border border-[var(--mc-line)] bg-[var(--mc-paper)] p-6 text-[15px] text-[var(--mc-muted)]">Loading current products...</div>}
      </div>
    </section>
  </>;
}
