...
import { ArrowRight, FileQuestion, FileText, MapPin, Package, Palette, RefreshCw, UserRound } from "lucide-react";
  ...
  return (
    <div>
      ...
      <div className="mt-7 grid gap-5 xl:grid-cols-2">
        <Records title="Orders" items={data.orders} empty="No orders yet." action="Browse products" href="/products" render={(item) => <><span><strong>{item.orderNumber}</strong><small className="mt-1 block text-[var(--mc-muted)]">{labelStatus(item.status)} / {date(item.createdAt)}</small></span><strong>{formatInr(item.total)}</strong></>} />
        <Records title="Quotes" items={data.quotes} empty="No quote requests yet." action="Open quote basket" href="/quote" render={(item) => <><span><strong>{item.quoteNumber}</strong><small className="mt-1 block text-[var(--mc-muted)]">{labelStatus(item.status)} / {date(item.createdAt)}</small></span><strong>{formatInr(item.total)}</strong></>} />
