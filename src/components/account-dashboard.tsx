...
export function AccountDashboard() {
  ...
  return (
    <div>
      ...
      <section className="mt-6 grid gap-px overflow-hidden border border-[var(--mc-line)] bg-[var(--mc-line)] sm:grid-cols-5">
        ...
        <div className="bg-white p-4"><ProfileValue label="Available balance" value={data.profileComplete && data.customer ? formatInr(data.customer.availableCredit) : "Complete profile"} />{!data.profileComplete ? <Link href="/account/profile" className="mt-2 inline-flex items-center gap-1 text-xs font-bold text-[var(--mc-accent)]">Complete profile <ArrowRight size={13} /></Link> : null}</div>
      </section>
...
function AccountSkeleton() { return <div className="animate-pulse py-8"><div className="h-8 w-56 rounded bg-[#dce4f0]" /><div className="mt-3 h-4 w-96 max-w-full rounded bg-[#e6ebf3]" /><div className="mt-8 grid gap-3 sm:grid-cols-3">{Array.from({ length: 3 }, (_, index) => <div key={index} className="h-28 rounded-lg border border-[var(--mc-line)] bg-white" />)}</div><div className="mt-5 grid gap-5 xl:grid-cols-2">{Array.from({ length: 4 }, (_, index) => <div key={index} className="h-44 rounded-lg border border-[var(--mc-line)] bg-white" />)}</div></div>; }
