...
// so the PDF export renders exactly the same list.
const customServiceIcons: Record<string, LucideIcon> = {
  books: BookOpen,
  diaries: Calendar,
  "bill-books": Receipt,
  "presentation-folders": FileSpreadsheet,
  packaging: Package,
  "paper-bags": ShoppingBag,
  "medical-files": FolderOpen,
  "security-vouchers": ShieldCheck,
};
const customCommercialServices = catalogCustomServices;
          ...
          <section className="space-y-6 pt-4 page-break-inside-avoid">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 print-grid-cols-2 print:gap-3">
              {customCommercialServices.map((service, idx) => {
                const IconComponent = customServiceIcons[service.key];
                return (
                  <div
                    key={idx}
