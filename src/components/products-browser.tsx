...
export function ProductsBrowser() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      setLoading(true);
      setError("");
      fetch(`/api/products?${params}`, { signal: controller.signal }).then(async (response) => ({ response, payload: await response.json().catch(() => null) })).then(({ response, payload }) => {
  ...
  }, [category, orderable, page, query, quoteable, requestVersion]);
  ...
  return <main className="mc-storefront min-h-screen bg-[var(--mc-surface)] text-[var(--mc-ink)]">
    <div className="mx-auto max-w-[1440px] px-4 py-7 lg:px-8 lg:py-10">
      ...
      <div className="mt-5 grid gap-3 lg:grid-cols-[minmax(0,1fr)_auto]">
        <label className="flex min-w-0 items-center gap-3 rounded-lg border border-[var(--mc-line)] bg-[var(--mc-paper)] px-4 shadow-sm"><Search size={18} className="shrink-0 text-[var(--mc-muted)]" /><input value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder="Search visiting cards, brochures, stickers..." className="min-w-0 flex-1 bg-transparent py-3.5 text-[15px] outline-none" />{query ? <button type="button" onClick={() => { setQuery(""); setPage(1); }} className="grid size-9 place-items-center rounded-full" aria-label="Clear search"><X size={17} /></button> : null}</label>
        {hasFilters ? <button type="button" onClick={clear} className="rounded-full border border-[var(--mc-line)] bg-[var(--mc-paper)] px-5 py-3 text-sm font-bold text-[var(--mc-accent)]">Clear filters</button> : null}
      </div>

      <div className="mt-4 border-y border-[var(--mc-line)] py-3">
        <div className="flex items-center gap-2 overflow-x-auto"><span className="mr-2 inline-flex shrink-0 items-center gap-2 text-xs font-bold uppercase text-[var(--mc-muted)]"><SlidersHorizontal size={15} />Categories</span><button type="button" onClick={() => { setCategory(""); setPage(1); }} className={`shrink-0 border-b-2 px-3 py-2 text-[15px] font-semibold ${!category ? "border-[var(--mc-accent)] text-[var(--mc-ink)]" : "border-transparent text-[var(--mc-muted)]"}`}>All products</button>{categories.map((item) => <button type="button" key={item.id} onClick={() => { setCategory(item.slug); setPage(1); }} className={`shrink-0 border-b-2 px-3 py-2 text-[15px] font-semibold ${category === item.slug ? "border-[var(--mc-accent)] text-[var(--mc-ink)]" : "border-transparent text-[var(--mc-muted)]"}`}>{item.name}</button>)}</div>
        <div className="mt-2 flex flex-wrap gap-5 text-[15px] text-[var(--mc-muted)]"><label className="flex items-center gap-2.5"><input type="checkbox" checked={orderable} onChange={(event) => { setOrderable(event.target.checked); setPage(1); }} className="size-[18px] accent-[var(--mc-accent)]" />Buy online</label><label className="flex items-center gap-2.5"><input type="checkbox" checked={quoteable} onChange={(event) => { setQuoteable(event.target.checked); setPage(1); }} className="size-[18px] accent-[var(--mc-accent)]" />Quote available</label></div>
      </div>
