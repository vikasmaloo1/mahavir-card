...
export function CustomerRecordDetail({ kind, id, upiVpa }: { kind: "order" | "quote"; id: string; upiVpa?: string }) {
  ...
  async function saveAsJob(item: Item) {
    if (!item.productId) return;
    const name = window.prompt("Name this job (e.g. \"ABC Company Visiting Card\")", item.jobName || item.description);
    if (!name) return;
    setSavingJobId(item.id); setSaveJobError("");
    try {
      const response = await fetch("/api/account/saved-jobs", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, productId: item.productId, configuration: item.configuration ?? {}, quantity: item.quantity }) });
      const payload = await response.json().catch(() => null);
      if (!response.ok || !payload?.success) throw new Error(payload?.error?.message ?? "This job could not be saved");
      setSavedJobIds((current) => new Set(current).add(item.id));
    } catch (caught) {
      setSaveJobError(caught instanceof Error ? caught.message : "This job could not be saved");
    } finally {
      setSavingJobId(null);
    }
  }
