...
export function ArtworkUploader({ productId, pricingRuleId, requirement, slot, showRequirements = true, configuration, artwork, onUploaded, onRemoved }: { productId: string; pricingRuleId: string | null; requirement: ArtworkRequirement; slot?: ArtworkSlot; showRequirements?: boolean; configuration: Record<string, string>; artwork: UploadedArtwork | null; onUploaded: (artwork: UploadedArtwork) => void; onRemoved: () => void }) {
  ...
  async function upload(file: File) {
    ...
    try {
      const startResponse = await fetch("/api/artworks/upload-url", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ productId, pricingRuleId, artworkSlotId: slot?.id ?? null, artworkSlotKey: slot?.slotKey ?? "MAIN", filename: file.name, contentType: file.type || "application/octet-stream", fileSize: file.size, replaceArtworkId: artwork?.id ?? null, configuration }) });
  ...
  return <section className="border border-[#d4dbe7] bg-[#fbfcff] p-4 sm:p-5"><input ref={input} type="file" accept=".cdr" className="sr-only" onChange={(event) => { const file = event.target.files?.[0]; event.currentTarget.value = ""; if (file) upload(file); }} />
    <div className="flex items-start justify-between gap-3"><div><p className="text-[13px] font-bold uppercase tracking-[0.13em] text-[#2457b8]">{slot?.name ?? "Upload artwork"}{slot && !slot.required ? " (optional)" : ""}</p><p className="mt-1 text-[15px] font-semibold text-[#162237]">{formatLabel()} only{maximumMb ? `, maximum ${maximumMb} MB` : ""}</p>{slot?.instructions ? <p className="mt-1 text-[13px] text-[#607089]">{slot.instructions}</p> : null}</div><FileBox size={23} className="text-[#2457b8]" /></div>
    {showRequirements ? <><div className="mt-3 grid gap-2 text-[13px] text-[#52647e] sm:grid-cols-3">{full ? <p><strong>Full design:</strong> {full}</p> : null}{safe ? <p><strong>Safe area:</strong> {safe}</p> : null}{final ? <p><strong>Final size:</strong> {final}</p> : null}</div>
    {pages.length ? <div className="mt-4 border-l-2 border-[#2457b8] pl-4"><p className="text-[13px] font-bold uppercase tracking-[0.1em] text-[#263753]">Required artwork files</p><ol className="mt-2 space-y-1.5 text-[15px] text-[#52647e]">{pages.map((page, index) => <li key={`${page.pageNumber}-${page.label}`}><span className="font-semibold text-[#162237]">{index + 1}. {page.label}</span>{page.colorMode ? ` (${page.colorMode})` : ""}{page.required === false ? " - when applicable" : ""}{page.notes ? <span className="block text-[13px]">{page.notes}</span> : null}</li>)}</ol></div> : null}
    {requirement.multiplePageInstructions ? <p className="mt-3 whitespace-pre-line text-[15px] leading-5 text-[#52647e]">{requirement.multiplePageInstructions}</p> : null}{requirement.additionalInstructions ? <p className="mt-3 whitespace-pre-line text-[15px] leading-5 text-[#52647e]">{requirement.additionalInstructions}</p> : null}</> : null}
    {!artwork ? <button type="button" onClick={choose} onDragOver={(event) => event.preventDefault()} onDrop={drop} disabled={phase === "uploading" || phase === "processing"} className="mt-4 flex w-full flex-col items-center justify-center border border-dashed border-[#9caed0] bg-white px-4 py-7 text-center hover:border-[#2457b8]"><UploadCloud size={28} className="text-[#2457b8]" /><span className="mt-2 text-[15px] font-bold text-[#162237]">Upload CDR artwork</span><span className="mt-1 text-[13px] text-[#607089]">Drop your CorelDRAW file here, or select it from your device.</span></button> : <div className="mt-4 border border-[#c8d7f1] bg-white p-4"><div className="flex items-start gap-3"><div className="grid size-10 shrink-0 place-items-center bg-[#edf3ff] text-[#2457b8]"><FileBox size={20} /></div><div className="min-w-0 flex-1"><p className="flex items-center gap-1 text-[15px] font-bold text-[#1f633d]"><CheckCircle2 size={16} />Artwork uploaded successfully</p><p className="mt-1 truncate text-[15px] font-semibold text-[#162237]">{artwork.originalFileName}</p><p className="mt-1 text-[13px] text-[#607089]">{bytes(artwork.fileSize)} / {artwork.fileType.toUpperCase()} / Ready for review</p></div></div><p className="mt-3 text-[13px] text-[#607089]">A visual document preview is not shown because CorelDRAW files are not browser-native images.</p><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={choose} className="inline-flex items-center gap-2 border border-[#aab9d1] px-3 py-2 text-[13px] font-bold text-[#2457b8]"><RefreshCw size={14} />Change file</button><button type="button" onClick={() => void remove()} className="inline-flex items-center gap-2 border border-[#e3c5c0] px-3 py-2 text-[13px] font-bold text-[#a53025]"><Trash2 size={14} />Remove</button></div></div>}
