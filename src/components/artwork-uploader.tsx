...
export function ArtworkUploader({ productId, pricingRuleId, requirement, configuration, artwork, onUploaded, onRemoved }: { productId: string; pricingRuleId: string | null; requirement: ArtworkRequirement; configuration: Record<string, string>; artwork: UploadedArtwork | null; onUploaded: (artwork: UploadedArtwork) => void; onRemoved: () => void }) {
  ...
  return <section className="border border-[#d4dbe7] bg-[#fbfcff] p-4 sm:p-5"><input ref={input} type="file" accept={formats.map((format) => `.${format.toLowerCase()}`).join(",")} className="sr-only" onChange={(event) => { const file = event.target.files?.[0]; event.currentTarget.value = ""; if (file) upload(file); }} />
    ...
    <div className="mt-3 grid gap-2 text-xs text-[#52647e] sm:grid-cols-3">{full ? <p><strong>Full design:</strong> {full}</p> : null}{safe ? <p><strong>Safe area:</strong> {safe}</p> : null}{final ? <p><strong>Final size:</strong> {final}</p> : null}</div>
    {pages.length ? <div className="mt-4 border-l-2 border-[#2457b8] pl-4"><p className="text-xs font-bold uppercase tracking-[0.1em] text-[#263753]">PDF page order</p><ol className="mt-2 space-y-1.5 text-sm text-[#52647e]">{pages.map((page, index) => <li key={`${page.pageNumber}-${page.label}`}><span className="font-semibold text-[#162237]">Page {index + 1}: {page.label}</span>{page.colorMode ? ` (${page.colorMode})` : ""}{page.required === false ? " - when applicable" : ""}{page.notes ? <span className="block text-xs">{page.notes}</span> : null}</li>)}</ol></div> : null}
    {requirement.multiplePageInstructions ? <p className="mt-3 whitespace-pre-line text-sm leading-5 text-[#52647e]">{requirement.multiplePageInstructions}</p> : null}{requirement.additionalInstructions ? <p className="mt-3 whitespace-pre-line text-sm leading-5 text-[#52647e]">{requirement.additionalInstructions}</p> : null}
    {!artwork ? <button type="button" onClick={choose} onDragOver={(event) => event.preventDefault()} onDrop={drop} disabled={phase === "uploading" || phase === "processing"} className="mt-4 flex w-full flex-col items-center justify-center border border-dashed border-[#9caed0] bg-white px-4 py-7 text-center hover:border-[#2457b8]"><UploadCloud size={28} className="text-[#2457b8]" /><span className="mt-2 text-sm font-bold text-[#162237]">Upload {formatLabel(formats)} artwork</span><span className="mt-1 text-xs text-[#607089]">Drop your artwork here, or select it from your device.</span></button> : <div className="mt-4 border border-[#c8d7f1] bg-white p-4"><div className="flex items-start gap-3"><div className="grid size-10 shrink-0 place-items-center bg-[#edf3ff] text-[#2457b8]"><FileBox size={20} /></div><div className="min-w-0 flex-1"><p className="flex items-center gap-1 text-sm font-bold text-[#1f633d]"><CheckCircle2 size={16} />Artwork uploaded successfully</p><p className="mt-1 truncate text-sm font-semibold text-[#162237]">{artwork.originalFileName}</p><p className="mt-1 text-xs text-[#607089]">{bytes(artwork.fileSize)} / {artwork.fileType.toUpperCase()} / Ready for review</p></div></div>{artwork.fileType.toLowerCase() === "cdr" ? <p className="mt-3 text-xs text-[#607089]">A visual document preview is not shown because CorelDRAW files are not browser-native images.</p> : null}<div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={choose} className="inline-flex items-center gap-2 border border-[#aab9d1] px-3 py-2 text-xs font-bold text-[#2457b8]"><RefreshCw size={14} />Change file</button><button type="button" onClick={() => void remove()} className="inline-flex items-center gap-2 border border-[#e3c5c0] px-3 py-2 text-xs font-bold text-[#a53025]"><Trash2 size={14} />Remove</button></div></div>}
