...
export function ArtworkUploader({ productId, pricingRuleId, requirement, configuration, artwork, onUploaded, onRemoved }: { productId: string; pricingRuleId: string | null; requirement: ArtworkRequirement; configuration: Record<string, string>; artwork: UploadedArtwork | null; onUploaded: (artwork: UploadedArtwork) => void; onRemoved: () => void }) {
  ...
  function upload(file: File) {
    ...
    body.append("file", file); body.append("productId", productId);
    if (pricingRuleId) body.append("pricingRuleId", pricingRuleId);
    if (artwork) body.append("replaceArtworkId", artwork.id);
    ...
    request.addEventListener("load", () => {
      if (request.status < 200 || request.status >= 300) { let message = "Upload failed. Please try again."; try { message = JSON.parse(request.responseText).error?.message || message; } catch { /* response is not JSON */ } setError(message); setPhase("failed"); return; }
      try { const payload = JSON.parse(request.responseText); onUploaded(payload.data); setProgress(100); setPhase("processing"); window.setTimeout(() => setPhase("idle"), 500); } catch { setError("Upload failed. Please try again."); setPhase("failed"); }
    });
    request.addEventListener("error", () => { setError("Connection interrupted. Please retry."); setPhase("failed"); });
    request.open("POST", "/api/artworks/upload"); request.send(body);
  }
