  ...
  return <main className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
    ...
    {data?.customer && data.profileComplete ? <>
      ...
      <div className="mt-7 grid gap-6 lg:grid-cols-[22rem_minmax(0,1fr)]">
        <form onSubmit={submit} className="h-fit rounded-xl border border-[var(--mc-line)] bg-white p-5 sm:p-6 shadow-sm"><div className="flex items-center gap-2"><CreditCard size={18} className="text-[var(--mc-accent)]" /><h2 className="font-bold text-lg text-[var(--mc-ink)]">Request balance top-up</h2></div><label className="mt-5 block"><span className="mb-2 block text-sm font-semibold text-[var(--mc-ink)]">Amount (₹)</span><input required min="1" step="0.01" type="number" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="e.g. 5000" className="w-full rounded-lg border border-[var(--mc-line)] px-3.5 py-3 outline-none focus:border-[var(--mc-accent)] transition-colors" /></label><button disabled={saving} className="mt-4 w-full rounded-full bg-[var(--mc-accent)] px-4 py-3 text-sm font-bold text-white shadow-sm hover:bg-[var(--mc-accent-dark)] transition-colors disabled:opacity-60">{saving ? "Submitting..." : "Request top-up"}</button>{message ? <p className="mt-3 text-sm text-[var(--mc-muted)]">{message}</p> : null}</form>
        <section className="rounded-xl border border-[var(--mc-line)] bg-white shadow-sm overflow-hidden"><h2 className="border-b border-[var(--mc-line)] p-5 font-bold text-lg text-[var(--mc-ink)]">Balance activity</h2>{data.transactions.length ? data.transactions.map((transaction) => <div key={transaction.id} className="flex items-center justify-between gap-4 border-b border-[var(--mc-line)] px-5 py-4 last:border-0 hover:bg-[var(--mc-surface)] transition-colors"><div><p className="font-semibold text-[var(--mc-ink)]">{transaction.transactionType.replaceAll("_", " ")}</p><p className="mt-1 text-xs text-[var(--mc-muted)]">{transaction.reference || "Account adjustment"} · {transaction.status}</p></div><strong className="text-[var(--mc-accent-dark)]">{formatInr(transaction.amount)}</strong></div>) : <p className="p-5 text-sm text-[var(--mc-muted)]">No balance activity yet.</p>}</section>
      </div>
    </> : null}
