...
export function CustomerAccountMenu() {
  return <details className="group relative">
    <summary className="flex cursor-pointer list-none items-center gap-1.5 px-2.5 py-2 min-h-[40px] rounded-full text-[var(--mc-muted)] hover:text-[var(--mc-ink)] hover:bg-[var(--mc-surface)] select-none">
      <UserRound size={17} /><span className="hidden sm:inline font-semibold">Account</span><ChevronDown size={14} className="transition group-open:rotate-180" />
    </summary>
    <div className="absolute right-0 top-full z-50 mt-2 w-52 rounded-xl border border-[var(--mc-line)] bg-white p-2 shadow-[0_16px_38px_rgba(16,33,63,0.14)]">
