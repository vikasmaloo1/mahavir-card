...
type Result = { order: { orderNumber: string }; payment: { method: string; status: string }; availableCredit: string | null };
...
export function CheckoutFlow() {
  ...
  if (result) return <div className="mx-auto max-w-xl py-20 text-center"><CheckCircle2 className="mx-auto text-[var(--mc-accent)]" size={44} /><p className="mt-6 text-lg font-bold">Order {result.order.orderNumber} created.</p><p className="mt-3 text-sm leading-6 text-[var(--mc-muted)]">{result.payment.method === "CREDIT" ? `The order is confirmed against your credit account. Available credit: ${formatInr(result.availableCredit)}.` : result.payment.method === "COD" ? "Cash on delivery is recorded and pending collection." : "Your Razorpay payment request is pending provider confirmation."}</p><Link href="/account" className="mt-7 inline-flex items-center gap-2 rounded-full bg-[var(--mc-accent)] px-5 py-3 text-sm font-bold text-white">View account <ArrowRight size={16} /></Link></div>;
