    ...
    <header className="sticky top-0 z-30 border-b border-[var(--mc-line)] border-t-[3px] border-t-[var(--mc-accent)] bg-[var(--mc-paper)]/95 shadow-[0_8px_28px_rgba(16,33,63,0.05)] backdrop-blur">
      <div className="mx-auto flex max-w-[1440px] items-center justify-between gap-3 px-4 py-3 lg:px-8">
        <Link href="/" className="flex shrink-0 items-center gap-3" aria-label="Mahavir Card home">
          <Image src="/api/branding/assets/logo.primary/file" alt="Mahavir Card mark" width={38} height={38} priority unoptimized className="size-[38px] rounded-full object-cover" />
          <span>
            <strong className="block text-[15px] text-[var(--mc-ink)]">Mahavir Card</strong>
            <span className="hidden text-[10px] font-medium text-[var(--mc-muted)] sm:block">Offset printing {"\u00b7"} Business cards {"\u00b7"} Ahmedabad</span>
          </span>
        </Link>
        ...
        <form action="/products" className="hidden min-w-0 max-w-xl flex-1 items-center border border-[var(--mc-line)] bg-white px-3 md:flex">
          ...
          <span className="border-l border-[var(--mc-line)] pl-3 text-[10px] font-bold uppercase tracking-[0.12em] text-[var(--mc-muted)]">Find</span>
        </form>
      ...
      <nav className="mx-auto hidden max-w-[1440px] gap-x-6 gap-y-2 px-4 pb-3 text-[13px] font-medium text-[var(--mc-muted)] md:flex lg:px-8" aria-label="Product categories">
      ...
      <nav className="flex gap-5 overflow-x-auto border-t border-[var(--mc-line)] px-4 py-2.5 text-xs font-semibold text-[var(--mc-muted)] md:hidden" aria-label="Mobile product categories">
