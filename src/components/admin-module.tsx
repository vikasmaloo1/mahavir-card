...
function ModuleForm({ section, item, products, saving, onSubmit, onCancel }: { section: ModuleKey; item: Row | null; products: Row[]; saving: boolean; onSubmit: (data: Record<string, unknown>) => Promise<void>; onCancel: () => void }) {
  ...
  const [form, setForm] = useState<Record<string, string>>(() => ({
    name: value("name"), slug: value("slug"), description: value("description"), sortOrder: value("sortOrder") || "0", code: value("code"), pricingType: value("pricingType") || "FIXED", priceConfiguration: asJson(item?.priceConfiguration), productId: value("productId"), ruleType: value("ruleType") || "PDF_PRICE_LIST", conditions: asJson(item?.conditions), priceFormula: asJson(item?.priceFormula), deliveryMethod: value("deliveryMethod") || "COURIER", stateCode: value("stateCode") || "*", price: value("price"), status: value("status") || defaultStatus(section), notes: value("notes"), internalNotes: value("internalNotes"), contactName: value("contactName"), email: value("email"), phone: value("phone"), companyName: value("companyName"), gstNumber: value("gstNumber"), message: value("message"), method: value("method") || "MANUAL", amount: value("amount"), orderId: value("orderId"), provider: value("provider"), providerOrderId: value("providerOrderId"), providerPaymentId: value("providerPaymentId"), password: "", quantity: "1", unitPrice: "0", itemDescription: "",
  }));
...
function ModuleFields({ section, form, toggles, products, update, setToggles, editing }: { section: ModuleKey; form: Record<string, string>; toggles: Record<string, boolean>; products: Row[]; update: (key: string) => (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => void; setToggles: React.Dispatch<React.SetStateAction<Record<string, boolean>>>; editing: boolean }) {
  ...
  if (section === "customers") return <div className="grid gap-4 sm:grid-cols-2">{field("Contact name", "contactName", { required: true })}{field("Company", "companyName", { required: true })}{field("Phone", "phone")}{field("GST number", "gstNumber")}{select("Customer type", "customerType", ["B2B", "B2C"])}{field("City", "city")}{field("State", "state")}{field("State code", "stateCode")}{field("Credit limit", "creditLimit")}{field("Available credit", "availableCredit")}{field("Wallet balance", "walletBalance")}{field("Payment terms (days)", "paymentTermsDays", { type: "number" })}{select("Credit enabled", "creditEnabled", ["false", "true"])}{select("Status", "status", ["ACTIVE", "INACTIVE"])}<p className="self-end text-xs leading-5 text-[#607089]">Email is tied to the customer account and cannot be changed here.</p></div>;
...
function buildPayload(section: ModuleKey, form: Record<string, string>, toggles: Record<string, boolean>, editing: boolean): Record<string, unknown> {
  ...
  if (section === "customers") return { contactName: form.contactName, companyName: form.companyName, phone: empty(form.phone), gstNumber: empty(form.gstNumber), customerType: form.customerType || "B2C", city: empty(form.city), state: empty(form.state), stateCode: empty(form.stateCode), creditEnabled: form.creditEnabled === "true", creditLimit: form.creditLimit || "0", availableCredit: form.availableCredit || "0", walletBalance: form.walletBalance || "0", paymentTermsDays: number(form.paymentTermsDays || "0"), status: form.status };
