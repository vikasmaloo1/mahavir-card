...
import { Check, ChevronLeft, ChevronRight, CircleAlert, Pencil, Plus, RefreshCw, Search, Trash2, X } from "lucide-react";
...
function Actions({ section, item, saving, actionLabel, onEdit, onDelete, onConvert }: { section: ModuleKey; item: Row; saving: boolean; actionLabel: string; onEdit: (item: Row) => void; onDelete: (item: Row) => void; onConvert: (item: Row) => void }) {
  ...
  if (["orders", "quotes", "customers", "payments", "artworks"].includes(section)) return <div className="mt-4 flex justify-end"><button type="button" onClick={() => onEdit(item)} className="border border-[#c9d2df] px-2.5 py-1.5 text-xs font-bold">{actionLabel}</button></div>;
...
function ModuleForm({ section, item, products, saving, onSubmit, onCancel }: { section: ModuleKey; item: Row | null; products: Row[]; saving: boolean; onSubmit: (data: Record<string, unknown>) => Promise<void>; onCancel: () => void }) {
  ...
  const [form, setForm] = useState<Record<string, string>>(() => ({
    name: value("name"), slug: value("slug"), description: value("description"), sortOrder: value("sortOrder") || "0", code: value("code"), pricingType: value("pricingType") || "FIXED", priceConfiguration: asJson(item?.priceConfiguration), productId: value("productId"), ruleType: value("ruleType") || "PDF_PRICE_LIST", conditions: asJson(item?.conditions), priceFormula: asJson(item?.priceFormula), deliveryMethod: value("deliveryMethod") || "COURIER", stateCode: value("stateCode") || "*", price: value("price"), status: value("status") || defaultStatus(section), notes: value("notes"), contactName: value("contactName"), email: value("email"), phone: value("phone"), companyName: value("companyName"), gstNumber: value("gstNumber"), message: value("message"), fileName: value("fileName"), storageKey: value("storageKey"), storageUrl: value("storageUrl"), method: value("method") || "MANUAL", amount: value("amount"), orderId: value("orderId"), provider: value("provider"), providerOrderId: value("providerOrderId"), providerPaymentId: value("providerPaymentId"), password: "", quantity: "1", unitPrice: "0", itemDescription: "",
  }));
...
function ModuleFields({ section, form, toggles, products, update, setToggles, editing }: { section: ModuleKey; form: Record<string, string>; toggles: Record<string, boolean>; products: Row[]; update: (key: string) => (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => void; setToggles: React.Dispatch<React.SetStateAction<Record<string, boolean>>>; editing: boolean }) {
  ...
  if (section === "artworks") return <div className="grid gap-4 sm:grid-cols-2">{select("Review status", "status", ["PENDING_REVIEW", "APPROVED", "CHANGES_REQUIRED", "REJECTED"])}{field("Storage key", "storageKey")}<div className="sm:col-span-2">{field("Stored file URL", "storageUrl", { type: "url" })}</div><div className="sm:col-span-2">{area("Review notes", "notes")}</div><p className="text-xs leading-5 text-[#607089] sm:col-span-2">This records existing storage metadata. Binary upload remains unavailable until an object-storage upload provider is configured.</p></div>;
...
function buildPayload(section: ModuleKey, form: Record<string, string>, toggles: Record<string, boolean>, editing: boolean): Record<string, unknown> {
  ...
  if (section === "artworks") return { status: form.status, notes: empty(form.notes), storageKey: empty(form.storageKey), storageUrl: empty(form.storageUrl) };
