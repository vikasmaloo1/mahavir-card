...
function ModuleForm({ section, item, products, saving, onSubmit, onCancel }: { section: ModuleKey; item: Row | null; products: Row[]; saving: boolean; onSubmit: (data: Record<string, unknown>) => Promise<void>; onCancel: () => void }) {
  ...
  const [toggles, setToggles] = useState<Record<string, boolean>>({ isActive: toggleValue("isActive", true), taxInclusive: toggleValue("taxInclusive", false), isDefault: toggleValue("isDefault", false), pricesTaxInclusive: toggleValue("pricesTaxInclusive", false) });
