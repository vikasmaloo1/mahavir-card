...
export async function GET(request: Request) {
  try {
    ...
    if (!customer) return jsonError("Complete your customer profile first", 422);
    ...
    return jsonOk({ customer: { walletBalance: customer.walletBalance, creditEnabled: customer.creditEnabled, creditLimit: customer.creditLimit, availableCredit: customer.availableCredit, paymentTermsDays: customer.paymentTermsDays }, transactions });
  } catch (error) {
