...
export async function GET(request: Request) {
  try {
    ...
    const [customer] = await db.select({ id: customers.id, companyName: customers.companyName, contactName: customers.contactName, phone: customers.phone }).from(customers).where(eq(customers.userId, session.user.id)).limit(1);
