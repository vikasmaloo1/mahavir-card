...
import { addresses, artworks, customers, inquiries, orders, payments, quotes } from "@/lib/db/schema";
...
export async function GET(request: Request) {
  try {
    ...
    const [customerOrders, customerQuotes, customerInquiries, customerArtwork, customerAddresses] = await Promise.all([
      customerId ? db.select({ id: orders.id, orderNumber: orders.orderNumber, status: orders.status, total: orders.total, createdAt: orders.createdAt, paymentStatus: payments.status }).from(orders).leftJoin(payments, eq(payments.orderId, orders.id)).where(eq(orders.customerId, customerId)).orderBy(desc(orders.createdAt)) : Promise.resolve([]),
