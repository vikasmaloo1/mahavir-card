...
import { artworkRequirements, categories, pricingRules, productAddons, products } from "@/lib/db/schema";
...
export async function GET(request: Request) {
  try {
    ...
    const [rules, addonRows, artworkRows] = productIds.length ? await Promise.all([
      db.select({ productId: pricingRules.productId, priceFormula: pricingRules.priceFormula }).from(pricingRules).where(and(inArray(pricingRules.productId, productIds), eq(pricingRules.isActive, true))),
    ...
    for (const rule of rules) { const amount = Number((rule.priceFormula as { amount?: string }).amount); if (Number.isFinite(amount) && (!startingPrices.has(rule.productId) || amount < startingPrices.get(rule.productId)!)) startingPrices.set(rule.productId, amount); }
    ...
    return jsonOk(data.map(({ product, category: categoryData }) => ({ ...product, category: categoryData, startingPrice: startingPrices.get(product.id) ?? null, hasAddons: productAddonsMap.has(product.id), hasArtworkRequirement: artworkMap.has(product.id) })));
  } catch (error) {
