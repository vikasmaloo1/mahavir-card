...
export async function GET(request: Request) {
  try {
    ...
    const items = pageData.map(({ product, category: categoryData }) => ({ ...product, category: categoryData, ...startingPrices.get(product.id), hasAddons: productAddonsMap.has(product.id), hasArtworkRequirement: artworkMap.has(product.id) }));
