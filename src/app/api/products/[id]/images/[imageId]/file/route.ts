...
export async function GET(_request: Request, ctx: RouteContext<"/api/products/[id]/images/[imageId]/file">) {
  try {
    ...
    return Response.redirect(await storage.getSignedDownloadUrl({ key: image.storageKey, filename: image.filename ?? "product-image", contentType: image.contentType ?? "application/octet-stream", disposition: "inline", expiresIn: 900 }), 302);
  } catch (error) {
