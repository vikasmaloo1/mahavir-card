...
export async function POST(request: Request) {
  ...
  try {
    ...
    const variant = (form.get("variant") as string) || "b2c";
    if (!(file instanceof File)) return jsonError("An image file is required for QR code", 422);

    await validateImageFile(file);
    newKey = storageKeys.qrCode(variant, file.name);
    ...
    await storage.uploadObject({
      ...
      visibility: "PUBLIC",
      metadata: { resource: "qr-code", variant, filename: file.name },
    });
