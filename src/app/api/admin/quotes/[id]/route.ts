...
export async function PATCH(request: Request, ctx: RouteContext<"/api/admin/quotes/[id]">) {
  try { await requireRole(request, ["ADMIN"]); const { id } = await ctx.params; const input = await readBody(request, adminQuoteUpdateSchema); const [existing] = await db.select().from(quotes).where(eq(quotes.id, id)).limit(1); if (!existing) return jsonError("Quote not found", 404); if (input.status && !canTransition("quote", existing.status, input.status)) return jsonError(`Cannot move a quote from ${existing.status} to ${input.status}`, 409); const [quote] = await db.update(quotes).set({ ...input, updatedAt: new Date() }).where(eq(quotes.id, id)).returning(); return quote ? jsonOk(quote) : jsonError("Quote not found", 404); } catch (error) { return error instanceof Response ? error : handleApiError(error); }
}
