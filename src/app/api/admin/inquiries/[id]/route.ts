...
const inquiryUpdateSchema = z.object({ status: z.enum(["NEW", "CONTACTED", "QUALIFIED", "QUOTATION_REQUESTED", "CONVERTED", "CLOSED", "LOST"]).optional(), message: z.string().trim().min(5).max(3000).optional(), internalNotes: z.string().trim().max(5000).nullable().optional() });
