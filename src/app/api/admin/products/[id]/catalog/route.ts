...
import { addons, pricingRules, productAddons, productContentItems, productContentSections, productDeliveryRules, productImages, products } from "@/lib/db/schema";
import { requireRole } from "@/lib/permissions";
import { adminPricingSchema, productAddonSchema, productContentItemSchema, productContentSectionSchema, productDeliveryRuleSchema, productImageSchema } from "@/lib/validation";

const resourceSchema = z.enum(["IMAGE", "SECTION", "SECTION_ITEM", "ADDON", "DELIVERY_RULE", "PRICING_RULE"]);
...
export async function GET(request: Request, ctx: RouteContext<"/api/admin/products/[id]/catalog">) {
  try {
    ...
    const [images, sections, contentItems, mappedAddons, deliveryRules, rules] = await Promise.all([
    ...
    return jsonOk({ images, sections: sections.map((section) => ({ ...section, items: contentItems.filter((item) => item.sectionId === section.id) })), addons: mappedAddons, deliveryRules, pricingRules: rules });
  } catch (error) {
...
export async function POST(request: Request, ctx: RouteContext<"/api/admin/products/[id]/catalog">) {
  try {
    ...
    if (input.resource === "ADDON") {
      const [mapping] = await db.insert(productAddons).values({ ...productAddonSchema.parse(input.data), productId }).returning();
...
export async function PATCH(request: Request, ctx: RouteContext<"/api/admin/products/[id]/catalog">) {
  try {
    ...
    if (input.resource === "ADDON") {
      const [mapping] = await db.update(productAddons).set({ ...productAddonSchema.partial().parse(input.data), updatedAt: new Date() }).where(and(eq(productAddons.id, input.id!), eq(productAddons.productId, productId))).returning();
