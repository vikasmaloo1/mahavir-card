...
import { artworks, inquiries, orders, payments, quotes } from "@/lib/db/schema";
...
export async function GET(request: Request) {
  try {
    ...
    const [ordersToday, pendingOrders, pendingQuotes, newInquiries, pendingPayments, pendingArtwork, recentOrders, recentQuotes] = await Promise.all([
    ...
    return jsonOk({ ordersToday: Number(ordersToday[0]?.value ?? 0), pendingOrders: Number(pendingOrders[0]?.value ?? 0), pendingQuotes: Number(pendingQuotes[0]?.value ?? 0), newInquiries: Number(newInquiries[0]?.value ?? 0), pendingPayments: Number(pendingPayments[0]?.value ?? 0), pendingArtwork: Number(pendingArtwork[0]?.value ?? 0), recentOrders, recentQuotes });
  } catch (error) {
