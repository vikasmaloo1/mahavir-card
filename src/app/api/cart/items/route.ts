...
export async function POST(request: Request) {
  try {
    ...
    const [item] = await db.insert(cartItems).values({ cartId: cart.id, productId: input.productId, quantity: input.quantity, configuration: input.configuration }).returning();
