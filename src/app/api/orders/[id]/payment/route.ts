...
export async function POST(request: Request, ctx: RouteContext<"/api/orders/[id]/payment">) {
  try { const session = await getSession(request); if (!session) return jsonError("Authentication required", 401); const { id } = await ctx.params; const input = await readBody(request, paymentSchema); if (input.orderId !== id) return jsonError("Order mismatch", 422); if (input.method === "CREDIT") return jsonError("Credit must be reserved during secure checkout", 422); const [ownedOrder] = await db.select({ order: orders, customer: customers }).from(orders).innerJoin(customers, eq(orders.customerId, customers.id)).where(and(eq(orders.id, id), eq(customers.userId, session.user.id))).limit(1); if (!ownedOrder) return jsonError("Order not found", 404); const amount = ownedOrder.order.total; const intent = createPaymentIntent(input.method, amount); const [payment] = await db.insert(payments).values({ orderId: id, customerId: ownedOrder.customer.id, method: input.method, amount, status: intent.status, provider: intent.provider }).onConflictDoUpdate({ target: payments.orderId, set: { method: input.method, amount, status: intent.status, provider: intent.provider, updatedAt: new Date() } }).returning(); return payment ? jsonOk(payment, 201) : jsonError("Payment was not created", 500); } catch (error) { return handleApiError(error); }
}
