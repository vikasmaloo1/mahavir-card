...
export default async function ProductPage({ params }: PageProps<"/catalog/[slug]">) {
  ...
  return <main className="mc-storefront min-h-screen bg-[var(--mc-surface)] text-[var(--mc-ink)]"><StorefrontHeader />
    <div className="mx-auto max-w-[1440px] px-4 py-6 xl:px-8"><div className="flex flex-wrap items-center justify-between gap-3"><a href="/products" className="inline-flex items-center gap-2 text-sm font-semibold text-[var(--mc-muted)] hover:text-[var(--mc-accent)]"><ArrowLeft size={16} /> All products</a><p className="text-xs font-semibold text-[var(--mc-muted)]">{descriptor}</p></div>
